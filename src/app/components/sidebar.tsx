...
export default function Sidebar({ closeDrawer }: { closeDrawer?: () => void }) {
  ...
  const links = [
    ...
    { href: "/projects", label: "My Projects", icon: ShareAltOutlined, color: "text-blue-600" },
    { href: "/skills", label: "My Stack", icon: BugOutlined, color: "text-blue-600" },
    { href: "/contact", label: "Contact Me", icon: CustomerServiceOutlined, color: "text-blue-600" },
    { href: "/blog", label: "My Blog", icon: BookOutlined, color: "text-blue-600" },
  ];
          ...
          return (
            <div
              ...
              className={`my-4 mx-auto rounded-lg py-8 px-12 transition-all flex items-center space-x-3 ${
                isActive
                  ? `bg-gray-100 translate-x-2.5 ${color} font-bold`
                  : "bg-white hover:bg-gray-50 text-blue-400"
              }`}
            >
              <Link
                href={href}
