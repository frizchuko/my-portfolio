      ...
      <Drawer title="Menu" placement="right" onClose={() => setOpen(false)} open={open} width={200} className="overflow-x-hidden">
        ...
        {links.map(({ href, label, icon: IconComponent, color }) => {
          ...
          return (
            <div
              key={href}
              className={`my-4 mx-auto rounded-lg py-8 px-10 transition-all ${
                isActive ? "bg-gray-200 !text-neutral-900 font-bold" : "bg-white hover:bg-gray-50 !text-neutral-900 hover:text-gray-900 font-bold"
              }`}
            >
